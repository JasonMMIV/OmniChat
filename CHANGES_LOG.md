...
- **Status**: 完成——`flutter test` full suite **432 tests passed**（v1.18.0 為 404 → +28：`settings_provider_claude_prompt_caching_test.dart` 12、`claude_prompt_caching_test.dart` 11、`deepseek_claude_compat_test.dart` 5）；`flutter analyze` 修改檔案 **no new errors/warnings**（新檔案 0 issues；既有檔案僅既有 withOpacity/unnecessary_cast 等，與 HEAD 相同）。
- **Version**: pubspec `1.18.1+91`；installer.iss 1.18.1（`OmniChat_windows_v1.18.1_setup`）。
- **Files Modified**: `lib/core/providers/settings_provider.dart`、`lib/core/services/api/chat_api_service.dart`、`lib/features/provider/pages/provider_detail_page.dart`、`lib/desktop/desktop_settings_page.dart`、`lib/l10n/*.arb`（+6 keys × 4 語系）＋ `app_localizations*.dart`（gen-l10n 重新生成）、`pubspec.yaml`、`installer.iss`、`CHANGES_LOG.md`。
- **Tests**: `test/settings_provider_claude_prompt_caching_test.dart`（NEW，12 案例：resolveClaudePromptCachingTtl 正常值/trim+lowercase/非法值回退／claudePromptCacheControl 5m 無 ttl、1h 帶 ttl／fromJson 預設 false+5m、讀取值、非法 ttl 正規化／toJson round-trip、非法 ttl 正規化／copyWith 保留／defaultsFor 五分支皆 disabled+5m）。`test/claude_prompt_caching_test.dart`（NEW，11 案例，HttpServer 端到端（sendMessageStream 主體路徑＋generateText 簡單 body 路徑）：Claude provider 啟用→`cache_control {type: ephemeral}`、1h→含 `ttl: '1h'`、停用/預設→無；OpenRouter Claude 模型→有、1h→有 ttl、非 Claude 模型→無、停用→無、非 OpenRouter OpenAI provider→無；簡單 body 兩路徑各有 1 案例）。`test/deepseek_claude_compat_test.dart`（NEW，5 案例：baseUrl `api.deepseek.com` 且 modelId 不含 deepseek→thinking `{type: enabled}`＋output_config `{effort: high}`（HttpOverrides proxy 保留 baseUrl 偵測）、provider id 含 deepseek、provider name 含 deepseek、modelId-only 相容、非 DeepSeek Claude provider 維持 adaptive 行為）。
...
- **193a** Images API 移植（`chat_api_service.dart`）：
  ...
  - 比例轉換 `_imageApiSizeParam`：custom body（`size`/`image_size`/`aspect_ratio`）優先 → `auto`（依來源：gpt-image-* 送 `size: "auto"`，其餘**省略注入**——Gemini/Nano Banana 編輯即自動保持來源比例）→ `useAspectRatioParam` 覆寫直傳 `aspect_ratio` 字串 → `size` 表格（1:1→1024x1024、3:4→1024x1360、4:3→1360x1024、2:3→1024x1536、3:2→1536x1024、16:9→1792x1024、9:16→1024x1792）→ 預設 size；dall-e-3 的 3:4/2:3→1024x1792、4:3/3:2→1792x1024 回退最接近合法值並在回覆 markdown 附加附註（service 無 BuildContext，不經 SnackBar）。
...
- **193c** 圖片比例選擇器（Phase 2）：
  ...
  - `lucide_adapter.dart` 新增 `Lucide.Ratio`（`2026-08-13` 由 `frame` 改為套件 3.1.15 內建的 `ratio` 交叉矩形比例符號）；`chat_input_button_catalog.dart` 在 `model` 之後新增 `imageRatio` 規格與預設順序（`chatInputButtonEffectiveOrder([])` 必須等於 default order 的測試不變量）。
  - `chat_input_bar.dart`：僅當 `ChatApiService.isOpenAIImageOutputModel` 為 true（**「是否圖片模型」的單一標準：kind == openai 且輸出含圖片，與開關無關**）顯示比例按鈕——關閉 Images API 開關後模型仍可能出圖，按鈕不消失（`Lucide.Ratio` 圖示 + tooltip 顯示目前比例的在地化標籤）；比例選項：**自動（依來源）**、1:1、3:4、4:3、2:3、3:2、16:9、9:16（`2026-08-13` 新增自動與 2:3/3:2；auto 預覽為方框＋A 提示）；行動版 bottom sheet、桌面版 anchored popover（等比例預覽方框），overflow 進 `+` 選單時回退居中 dialog；Gemini 內嵌出圖不顯示。
- **Status**: 全部完成——`flutter test` full suite **270 tests passed**（v1.16.0 為 230 → +40：`openai_images_api_test.dart` 37、`settings_provider_image_ratio_test.dart` 3）；`flutter analyze` 修改檔案 **no new errors**（全專案僅既有 vendored `speech_to_text_windows` example 2 個 error）。實機端到端：文生圖正常（Agnes 取消勾選「使用 aspect_ratio 參數」後生成成功，預設 `size` 注入無害）；Gemini 3.1 Flash Lite Image 於關閉 Images API 開關時可編輯（聊天補全圖進圖出）；Agnes Image 2.1 Flash 圖生圖原 503（無 `/images/edits` 端點）已由 Agnes generations 路徑修復。
- **Version**: pubspec `1.17.0+81`；installer.iss 1.17.0（`OmniChat_windows_v1.17.0_setup`）。
- **Files Modified**: `lib/core/services/api/chat_api_service.dart`、`lib/features/model/widgets/model_detail_sheet.dart`、`lib/desktop/model_edit_dialog.dart`、`lib/core/providers/model_provider.dart`、`lib/core/providers/settings_provider.dart`、`lib/icons/lucide_adapter.dart`、`lib/features/home/utils/chat_input_button_catalog.dart`、`lib/features/home/widgets/chat_input_bar.dart`、`lib/core/services/chat/chat_turn_service.dart`、`lib/features/home/controllers/chat_actions.dart`、`lib/features/home/services/ocr_service.dart`、`lib/features/home/services/translation_service.dart`、`lib/l10n/*.arb`（+13 keys × 4 語系，`flutter gen-l10n` 重新生成）、`pubspec.yaml`（+`http_parser: ^4.1.2`、version `1.17.0+81`）、`installer.iss`、`CHANGES_LOG.md`、`導入 kelivo 圖片生成模型支援.md`（計畫文件同步實作細節）。
- **Tests**: `test/openai_images_api_test.dart`（NEW，37 案例：輸出模式判斷（id 推斷/override 覆寫/`useImagesApi: false` 關閉/預設開啟/非圖片模型不可 opt-in/非 OpenAI kind 拒絕）、`isOpenAIImageOutputModel`（含開關關閉仍為圖片模型）、`isOpenAIImageEditModel`（input+output 為編輯模型/dall-e-3、sensenova 非編輯/override 覆寫/非 OpenAI kind 拒絕）、開關關閉走 `/chat/completions` 且注入 size、生成/編輯 multipart、jpeg content-type、結構化輸入、dall-e-3 編輯拒絕、Agnes host 圖生圖走 generations（本地檔→Data URI、遠端 URL 直傳）、auto（gpt-image 送 `size: "auto"` / 非 gpt-image 省略注入，含 useAspectRatioParam 併存）、2:3/3:2 換算（1024x1536/1536x1024）、dall-e-3 2:3 回退附註、base64 存檔/失敗、疊代編輯、非 2xx、size 注入、custom body 優先、dall-e-3 回退附註、`aspect_ratio` 直傳）、`test/settings_provider_image_ratio_test.dart`（NEW，3 案例：預設/round-trip/notify）。
...
- **Files Modified**: `lib/features/home/widgets/chat_input_bar.dart`、`CHANGES_LOG.md`。
