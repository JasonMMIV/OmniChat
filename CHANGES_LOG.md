...
## [v1.5.14] - 2026-05-27: Consolidated Release for MCP, Voice Chat, and Input Bar Updates
...
- **Details**:
  ...
  - **Execution Guards**: QuickJS-backed platforms use runtime timeout and stack-size guards. The tool also rejects empty, oversized, and obvious infinite-loop snippets before evaluation.
...
- **Purpose**: Allow users to paste AI messages with Rich Text formatting (bold, italics, headers) into other applications.
- **Files Modified**:
  - `lib/features/chat/widgets/chat_message_widget.dart`
  - `lib/features/chat/widgets/select_copy_sheet.dart`
  - `lib/features/chat/pages/select_copy_page.dart`
  - `lib/desktop/select_copy_dialog.dart`
- **Details**:
  - Integrated `super_clipboard` and `markdown` rendering.
  - Generates HTML metadata mapped to OS clipboards. Passes a plain text fallback for simple editors.

### 89. Windows Export Bug Fix
- **Purpose**: Fix the issue where trying to export a chat backup to a file on Windows displayed a "Try that again. We couldn't show you all the ways you could share." error.
- **Files Modified**:
  - `lib/features/backup/pages/backup_page.dart`
- **Details**:
  - Detected `Platform.isWindows` (and other desktops) to replace `Share.shareXFiles` with `FilePicker.platform.saveFile`.
  - Opens native OS "Save As" dialogue and copies the generated .zip backup there manually, preserving reliability.

### 90. Windows Build & Installer Fixes
- **Purpose**: Resolve critical compilation errors on Windows and repair installer versioning.
- **Files Modified**:
  - `windows/runner/Runner.rc`
  - `windows/CMakeLists.txt`
  - `installers/omnichat_setup.iss`
- **Details**:
  - **Runner.rc**: Replaced hardcoded `"1.0.0.1"` with Flutter's global `VERSION_AS_STRING` macro to dynamically sync the executable version with `pubspec.yaml`.
  - **CMakeLists.txt**: Added `OPTIONAL` keyword to `native_assets` directory installation, preventing build failures when assets are absent.
  - **Inno Setup**: Recompiled the Windows Installer `.exe` with the fully patched executable and updated script version to v1.4.9.
