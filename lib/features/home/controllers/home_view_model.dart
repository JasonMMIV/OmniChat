...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<String?> compressContext({
    ...
    try {
      ...
      final workspaceConfig = _chatService.getConversationWorkspaceConfig(
        convo.id,
      );
  ...
  String getClearContextLabel(
    ...
    final configured = (assistant?.limitContextMessages ?? true)
        ...
        : 0;
    final convo = currentConversation;
    final completeMessages = convo != null
        ? _chatService.getMessages(convo.id)
        : const <ChatMessage>[];
  ...
  Future<void> _maybeGenerateTitleFor(
    ...
    try {
      final title = (await ChatApiService.generateText(
        ...
        modelId: mdlId,
        prompt: prompt,
        thinkingBudget: settings.titleGenerationThinkingBudgetFor(
