...
class ChatApiService {
      ...
      if (role == 'tool') {
        ...
        if (content is String && content.isNotEmpty) {
          return <String, dynamic>{
            ...m,
            'content': _truncateToolResultText(content),
          };
        }
        if (content is List) {
            ...
            if (block is Map &&
              ...
              if (text.length > 32768) {
                modified.add(<String, dynamic>{
                  ...block,
                  'text': _truncateToolResultText(text),
                });
                changed = true;
                continue;
              }
        ...
        for (final item in contentList) {
          if (item is Map && item['type'] == 'tool_result') {
            ...
            if (tc is String && tc.isNotEmpty) {
              modified.add(<String, dynamic>{
                ...item,
                'content': _truncateToolResultText(tc),
              });
              changed = true;
              continue;
            }
            if (tc is List) {
              ...
              for (final block in tc) {
                if (block is Map &&
                  ...
                  final text = block['text'] as String;
                  if (text.length > 32768) {
                    innerModified.add(<String, dynamic>{
                      ...block,
                      'text': _truncateToolResultText(text),
                    });
                    innerChanged = true;
  ...
  static Future<String?> _saveInlineImageToFile(
    String mime,
    String data,
  ) async {
    try {
    ...
    try {
      if (kind == ProviderKind.openai) {
        ...
        final body = {
          'model': upstreamModelId,
          'max_tokens': 512,
          if (!usesAdaptive) 'temperature': 0.3,
          'messages': [
            {'role': 'user', 'content': safePrompt},
          ],
  ...
  static bool _isOff(int? budget) => ReasoningBudget.isOff(budget);

  static bool _isKimiK3Model(String modelId) => RegExp(
    r'(?:^|[/_:@])kimi-k3(?:$|[-.])',
    caseSensitive: false,
  ).hasMatch(modelId);

  static void _removeKimiK3SamplingParams(
    Map<String, dynamic> body,
    String modelId,
  ) {
  ...
  static String _openAIEffortForBudget(int? budget, String modelId) {
    ...
    if (base == 'high' && budget != null) {
      if (budget >= ReasoningBudget.max && capabilities.supportsMax) {
        requested = 'max';
      } else if (budget >= ReasoningBudget.xhigh &&
          capabilities.supportsXhigh) {
        requested = 'xhigh';
      }
    }
    requested = capabilities.normalizeOpenAiEffort(requested);
    if (requested == 'max' && !capabilities.supportsMax) {
      return capabilities.supportsXhigh ? 'xhigh' : 'high';
    }
    if (requested == 'xhigh' && !capabilities.supportsXhigh) {
      return 'high';
    }
    return requested;
  }
  ...
  static String _claudeEffortForBudget(int? budget, String modelId) {
    ...
    final requested = budget <= 32000
        ? 'high'
        : (budget <= ReasoningBudget.xhigh ? 'xhigh' : 'max');
    if (requested == 'max' && !capabilities.supportsMax) {
      return capabilities.supportsXhigh ? 'xhigh' : 'high';
    }
    if (requested == 'xhigh' && !capabilities.supportsXhigh) {
      return capabilities.supportsMax ? 'max' : 'high';
    }
    return requested;
  }
  ...
  static Map<String, dynamic>? _claudeThinkingConfig(
    ...
    if (budget != null && budget > 0) {
      final effectiveBudget =
          capabilities.supportsXhigh ||
              capabilities.supportsMax ||
              budget <= ReasoningBudget.heavy
          ? budget
          : ReasoningBudget.heavy;
      return {'type': 'enabled', 'budget_tokens': effectiveBudget};
    }
    // Auto on legacy Claude leaves the provider default untouched.
    return null;
  }
          ...
          if (onToolCall != null && toolAcc.isNotEmpty) {
            ...
            while (true) {
              ...
              if ((finishReason2 == 'tool_calls' || toolAcc2.isNotEmpty) &&
                ...
                currentMessages = _truncateToolResultsInMessages(
                  currentMessages,
                );
                currentMessages = [
                  ...currentMessages,
                ...
                if (output is List) {
                  ...
                  for (final it in output) {
                    ...
                    if (it['type'] == 'message') {
                      ...
                      if (b64.isNotEmpty) {
                        final savedPath = await _saveInlineImageToFile(
                          'image/png',
                          b64,
                        );
  ...
  static Stream<ChatStreamChunk> _sendGoogleStream(
    ...
    if (!stream) {
      ...
      if (isVertex &&
          ...
          (config.location?.isNotEmpty == true)) {
        url =
            'https://aiplatform.googleapis.com/v1/projects/${config.projectId}/locations/${config.location}/publishers/google/models/$modelId:generateContent';
      } else {
    ...
    if (config.vertexAI == true &&
      ...
      final proj = config.projectId!.trim();
      baseUrl =
          'https://aiplatform.googleapis.com/v1/projects/$proj/locations/$loc/publishers/google/models/$modelId:streamGenerateContent';
    } else {
      final base = config.baseUrl.endsWith('/')
          ? config.baseUrl.substring(0, config.baseUrl.length - 1)
          : config.baseUrl;
    ...
    while (true) {
      final gen = <String, dynamic>{
        ...
        if (isReasoning)
          'thinkingConfig': () {
            ...
            final isGemini3ProImage = modelId.contains(
              RegExp(r'gemini-3.*-pro-image(-preview)?', caseSensitive: false),
            );
            final isGemini3Pro = modelId.contains(
              RegExp(r'gemini-3.*-pro(-preview)?', caseSensitive: false),
            );
            final isGemini3FlashOrLite = modelId.contains(
              RegExp(
                r'gemini-3.*-(flash|lite)(-preview)?',
                caseSensitive: false,
              ),
            );
      ...
      Future<String> _takeBufferedImageMarkdown() async {
        ...
        final path = await _saveInlineImageToFile(
          _imageMime,
          _pendingImageData,
        );
  ...
  static Stream<ChatStreamChunk> _sendOpenAIImagesStream(
    ...
    final ratioNote = _openAIImagesRatioFallbackNote(
      config,
      modelId,
      extraBody,
      imageAspectRatio,
  ...
  static Future<Map<String, dynamic>> _sendOpenAIImageGeneration(
    http.Client client,
    ProviderConfig config,
    String modelId,
    String prompt, {
  ...
  static Future<Map<String, dynamic>> _sendOpenAIImageEdit(
    http.Client client,
    ProviderConfig config,
    String modelId,
    String prompt,
  ...
  static Future<String> _lastOpenAIImagePrompt(
    List<Map<String, dynamic>> messages,
  ) async {
  ...
  static Future<_OpenAIImagesInput> _openAIImagesInput(
    List<Map<String, dynamic>> messages,
    List<String>? userImagePaths,
  ) async {
  ...
  static _ImageRef? _lastAssistantImageBefore(
    List<Map<String, dynamic>> messages,
    int beforeIndex,
  ) {
  ...
  static List<_ImageRef> _extractOpenAIImageRefs(dynamic content) {
    if (content is List) {
  ...
  static void _addOpenAIStructuredImageRefs(List<_ImageRef> refs, dynamic value) {
  ...
  static void _addOpenAIStructuredImageData(
    List<_ImageRef> refs,
    dynamic data, {
  ...
  static _ImageRef _imageRefFromSource(String source) {
  ...
  static Future<http.MultipartFile> _openAIImageMultipartFile(
    _ImageRef ref,
  ) async {
    if (ref.kind == 'data') {
      final mime = _mimeFromDataUrl(ref.src);
      ...
      return http.MultipartFile.fromBytes(
        ...
        filename: 'image.${_extFromMime(mime)}',
  ...
  static MediaType _openAIImageMediaType(String mime) {
  ...
  static Map<String, String> _openAIImagesJsonHeaders(
    ProviderConfig config,
    String modelId, {
    Map<String, String>? extraHeaders,
  }) {
  ...
  static Map<String, String> _openAIImagesMultipartHeaders(
    ProviderConfig config,
    String modelId, {
    Map<String, String>? extraHeaders,
  }) {
  ...
  static void _applyOpenAIImagesExtraBody(
    Map<String, dynamic> body,
    ProviderConfig config,
    String modelId,
    Map<String, dynamic>? extraBody,
  ...
  static void _applyOpenAIImagesSize(
    ...
    if (imageAspectRatio == null || imageAspectRatio.isEmpty) return;
    final hasManual = body.keys.any(
      (key) => key == 'size' || key == 'image_size' || key == 'aspect_ratio',
    );
    if (hasManual) return;
    body.addAll(_imageApiSizeParam(imageAspectRatio, modelId, cfg));
  }
  ...
  static Map<String, String> _imageApiSizeParam(
    ...
    switch (ratio) {
      case '3:4':
        size = upstream == 'dall-e-3' ? '1024x1792' : '1024x1360';
        break;
      case '4:3':
  ...
  /// Returns a markdown note when the chosen ratio had to fall back for dall-e-3
  /// (3:4/4:3 are not supported by dall-e-3). Null otherwise.
  static String? _openAIImagesRatioFallbackNote(
    ProviderConfig cfg,
    String modelId,
    Map<String, dynamic>? extraBody,
    String? imageAspectRatio,
    ...
    final manual = <String>['size', 'image_size', 'aspect_ratio'].any(
      (key) =>
          _customBody(cfg, modelId).containsKey(key) ||
          (extraBody?.containsKey(key) ?? false),
    );
  ...
  static String _openAIImagesOutputMime(
    ProviderConfig config,
    String modelId,
    Map<String, dynamic>? extraBody,
  ) {
  ...
  static Map<String, dynamic> _decodeOpenAIImagesResponse(
  ...
  static Future<String> _openAIImagesResponseToMarkdown(
    ...
    for (final item in data) {
      ...
      final path = await _saveInlineImageToFile(outputMime, b64);
  ...
  static TokenUsage? _openAIImagesUsage(Map<String, dynamic> response) {
